...
export default function Dashboard() {
  ...
  for (const g of groups) {
    ...
    // Members joined (skip the creator who joined at the same time as group was created)
    for (const m of (g.members ?? [])) {
      ...
      if (Math.abs(joinedAt - createdAt) > 5000) {
        // joined more than 5s after group creation → it's a real add-member event
  ...
  return (
    <SafeAreaView className="flex-1 bg-base" edges={["top"]}>
      ...
      <View className="px-5 mt-4 gap-3">
        <View style={{ backgroundColor: net >= 0 ? "rgba(34,197,94,0.1)" : "rgba(244,63,94,0.1)", borderRadius: 20, borderWidth: 1, borderColor: net >= 0 ? "rgba(34,197,94,0.2)" : "rgba(244,63,94,0.2)", padding: 20 }}>
